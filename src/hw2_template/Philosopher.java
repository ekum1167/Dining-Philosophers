...
public class Philosopher extends Thread {
	...
	private int getRandomTimeInRange(int min, int max) {
	...
	private void barrierHandler(int id) {
		try {
			// Barrier approach - release its semaphore N - 1 times
			for(int i = 0; i < this.N; i++)
				if(id != i)
					barriers[id].release();
			// Acquire other semaphores N - 1 times
			for(int i = 0; i < this.N; i++)
				if(id != i)
					barriers[i].acquire();
		}
	...
	private void setThinking(){
		// Make the thread think with setting its state and making it sleep
		try {
			state[id] = THINKING;
			Thread.sleep(this.getRandomTimeInRange(0,10));
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
	}

	private boolean isForkAvailable(int leftNeig, int rightNeig){
		return state[id] == HUNGRY && state[rightNeig] != EATING && state[leftNeig] != EATING;
	}

	private void setHungry(int leftNeig, int rightNeig){

		// Set state to hungry
		state[id] = HUNGRY;
		table.Hungry_GUI(id);

		// Get Mutex for fork action possibility
		try {
			mutex.acquire();
		} catch (InterruptedException e) {
			e.printStackTrace();
		...
		// Check neighbours to see if taking fork is allowed,
		if(isForkAvailable(leftNeig, rightNeig))
		{
	...
	private void setEating(int leftNeig, int rightNeig, int leftNeigTwo, int rightNeigTwo){
		try {
			sem[id].acquire();
		} catch (InterruptedException e) {
			e.printStackTrace();
		...
		// Give the forks to the right neighbour and make it eat
		if(isNeighbourAvailable(rightNeig,rightNeigTwo))
			this.giveTheForksToNeighbour(rightNeig);

		// Give the forks to the left neighbour and make it eat
		if(isNeighbourAvailable(leftNeig, leftNeigTwo))
			this.giveTheForksToNeighbour(leftNeig);

		// Release Mutex
		mutex.release();
	}

	private boolean isNeighbourAvailable(int neig, int neigTwo){
		return state[neig] == HUNGRY && state[neigTwo] != EATING;
	}

	private void giveTheForksToNeighbour(int neig){
		table.ForkTake_GUI(neig);
		state[neig] = EATING;
		sem[neig].release();
	}
	...
	public void run() {
		...
		try {
			Thread.sleep(this.getRandomTimeInRange(1,10));
		} catch(InterruptedException e)
		{
			e.printStackTrace();
		}
		...
		// Barrier Approach
		this.barrierHandler(id);

		// Start Dining
		table.StartDining_GUI(id);

		// Get the neighbour information
		int rightNeighbour = (id + 1) % 5;
		int LeftNeighbour = (id + 4) % 5;
		int rightNeighbourTwo = (id + 2) % 5;
		int LeftNeighbourTwo = (id + 8) % 5;

		while(true){
			// Think first
			this.setThinking();

			// Now eat, use mutex for race conditions
			this.setHungry(LeftNeighbour, rightNeighbour);

			// Eating now
			this.setEating(LeftNeighbour, rightNeighbour, LeftNeighbourTwo, rightNeighbourTwo);
		}
	}
}
